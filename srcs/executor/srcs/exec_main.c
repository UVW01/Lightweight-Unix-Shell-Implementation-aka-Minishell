...
/*   Updated: 2022/06/23 17:40:03 by mnaimi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_env_vars **env_head)
{
	...
	if (node->cmd_flag == -1 && !ft_strstr_tl(BUILT_INS, node->cmd_split[0]))
		g_glob.exit = 127;
	else
	{
...
int	handle_execution(t_info *parsed_data, t_env_vars **env_head)
{
	...
	check_invalid_command(parsed_data->head);
	while (exec.node)
	{
		if ((exec.node->token == IN || exec.node->token == OUT || \
			...
			exec.input = exec.node->file_fd;
		else if (exec.node->token == OUT || exec.node->token == APPEND)
			exec.output = exec.node->file_fd;
